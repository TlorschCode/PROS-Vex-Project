/* ! THIS FILE IS A WORK IN PROGRESS. I USE THIS TO UPLOAD MY CODE WHENEVER MY VEX BATTERY DIES ! */
#include "main.h"
#include <iostream>
#include "vector"
#include <string>
...
void move_to(float tarx, float tary) {
	...
	while (auton_y || auton_x || auton_rot) {
		check_quit_program();
		left_speed = (((y_diff * cos(rot)) + (x_diff * sin(rot))) / ceil((rot_diff / 55))) - rot_diff;
		right_speed = (((y_diff * cos(rot)) + (x_diff * sin(rot))) / ceil((rot_diff / 55))) + rot_diff;
